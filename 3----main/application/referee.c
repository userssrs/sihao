...
uint16_t get_client_id(void)
{
    ...
    {
        return 0x100 + robot_state.robot_id;
    }
    else
    {
        return 0x164 + (robot_state.robot_id - 100);
    }
}


/**
  * @brief          获取敌方机器人的颜色   0：蓝色   1：红色
  * @param[in]      none
  * @retval         uint8_t
  */
uint8_t get_enermy_color(void)
{
    if (robot_state.robot_id < 100)
        return 0;
    else
...
/**
  ...
  * @retval         uint16_t
  */
uint16_t get_17mm_shoot_speed_limit(void)
{
